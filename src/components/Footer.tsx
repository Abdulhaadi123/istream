...
export default function Footer() {
  return (
    <footer className="bg-[var(--foreground)] pt-16 pb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="lg:col-span-1">
            <Link href="/" className="flex items-center gap-2.5 mb-4 group">
              <div className="w-8.5 h-8.5 rounded-lg overflow-hidden border border-white/10 relative">
                <Image
                  src="/logo.jpg"
                  alt="iStream Solution Logo"
                  fill
                  className="object-cover"
                />
              </div>
              <span className="font-[family-name:var(--font-space-grotesk)] text-xl font-bold text-white tracking-tight">
                iStream<span className="text-[var(--accent)]"> Solution</span>
              </span>
            </Link>
            <p className="text-sm text-slate-400 leading-relaxed mb-6 max-w-xs">
              Sourcing and managing elite remote engineering talent. End-to-end recruitment, automated payroll, and global operations.
            </p>
            <div className="flex gap-3">
              {socials.map((social) => (
                <a key={social.label} href={social.href} aria-label={social.label} className="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 hover:text-[var(--accent)] hover:border-[var(--accent)]/30 transition-all">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {footerLinks.services.map((link) => (
                <li key={link.name}><Link href={link.href} className="text-sm text-slate-400 hover:text-white transition-colors">{link.name}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm text-slate-400">
              <li><a href="mailto:hello@istreamsolution.com" className="hover:text-white transition-colors">hello@istreamsolution.com</a></li>
              <li className="leading-relaxed">Florida,<br />United States</li>
            </ul>
          </div>
        </div>

        <div className="border-t border-white/10 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-slate-500">© {new Date().getFullYear()} iStream Solution. All rights reserved.</p>
