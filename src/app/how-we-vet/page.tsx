...
export const metadata: Metadata = {
  title: "How We Vet Talent — iStream Solution",
  description: "Learn about iStream Solution's rigorous 5-stage vetting process to select and onboarding the top 3% of global software engineering talent.",
};
...
export default function VettingProcessPage() {
  return (
    <main className="pt-32 pb-20 relative min-h-screen">
      ...
      <div className="absolute top-[10%] left-[5%] w-[500px] h-[500px] bg-[var(--primary)]/5 rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute bottom-[20%] right-[5%] w-[500px] h-[500px] bg-[var(--accent)]/5 rounded-full blur-[140px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-3xl p-6 sm:p-8 mb-20 text-center relative overflow-hidden">
          <h2 className="text-sm font-bold text-[var(--foreground)] uppercase tracking-wider mb-6">Our Talent Pipeline Selection Funnel</h2>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              ...
              <div key={i} className="bg-[var(--background)] rounded-2xl border border-slate-200 p-4 flex flex-col justify-center">
                <span className="text-[10px] text-[var(--text-secondary)] font-semibold mb-1 truncate">{f.label}</span>
                <span className="font-[family-name:var(--font-space-grotesk)] text-xl sm:text-2xl font-extrabold text-[var(--foreground)]">{f.val}</span>
        ...
        <div className="space-y-12 mb-20">
          {vettingStages.map((stage, index) => (
            <div 
              key={stage.step} 
              className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-3xl p-6 sm:p-10 hover:border-slate-300 transition-all duration-300 flex flex-col lg:flex-row gap-6 lg:gap-10 items-stretch"
            >
              ...
              <div className="flex flex-col justify-between lg:w-4/12 gap-4">
                <div>
                  ...
                  <h3 className="font-[family-name:var(--font-space-grotesk)] text-lg sm:text-xl font-bold text-[var(--foreground)] mt-2">
                    {stage.title}
                  </h3>
                </div>
              ...
              <div className="flex flex-col justify-center lg:w-8/12 text-left">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {stage.details.map((detail, di) => (
                    <div key={di} className="flex items-center gap-2 text-xs text-slate-700 bg-slate-50 border border-slate-200 rounded-xl p-3">
        ...
        {/* CTA Bottom Block */}
        <div className="bg-gradient-to-br from-[var(--primary)]/10 to-[var(--accent)]/5 border border-[var(--card-border)] rounded-3xl p-8 sm:p-12 text-center relative overflow-hidden">
          <div className="relative z-10">
            <h2 className="font-[family-name:var(--font-space-grotesk)] text-2xl sm:text-3xl font-extrabold text-[var(--foreground)] mb-4 tracking-tight">
            ...
            <Link 
              href="/contact" 
              className="inline-flex items-center gap-2 px-8 py-3.5 rounded-full text-sm font-bold text-[var(--foreground)] bg-gradient-to-r from-[var(--primary)] to-[var(--accent)] hover:shadow-[0_0_28px_rgba(26,79,143,0.4)] transition-all duration-300 hover:-translate-y-0.5"
            >
